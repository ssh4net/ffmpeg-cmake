...
set(FFMPEG_NATIVE_BUILD_FFPLAY "AUTO" CACHE STRING "Build ffplay with the native CMake backend: AUTO builds when SDL2 is available, ON requires SDL2, OFF disables it.")
set_property(CACHE FFMPEG_NATIVE_BUILD_FFPLAY PROPERTY STRINGS AUTO ON OFF)
...
option(FFMPEG_NATIVE_ALL_REQUIRE_KNOWN_DEPENDENCIES "Keep the ALL component profile platform-clean by pruning components whose FFmpeg configure dependency tokens have no native CMake probe." ON)

string(TOUPPER "${FFMPEG_NATIVE_BUILD_FFPLAY}" FFMPEG_NATIVE_BUILD_FFPLAY)
if(FFMPEG_NATIVE_BUILD_FFPLAY STREQUAL "TRUE")
    set(FFMPEG_NATIVE_BUILD_FFPLAY ON)
elseif(FFMPEG_NATIVE_BUILD_FFPLAY STREQUAL "FALSE")
    set(FFMPEG_NATIVE_BUILD_FFPLAY OFF)
endif()
if(NOT FFMPEG_NATIVE_BUILD_FFPLAY STREQUAL "AUTO" AND
   NOT FFMPEG_NATIVE_BUILD_FFPLAY STREQUAL "ON" AND
   NOT FFMPEG_NATIVE_BUILD_FFPLAY STREQUAL "OFF")
    message(FATAL_ERROR "FFMPEG_NATIVE_BUILD_FFPLAY must be AUTO, ON, or OFF.")
endif()
set(FFMPEG_NATIVE_BUILD_FFPLAY "${FFMPEG_NATIVE_BUILD_FFPLAY}" CACHE STRING "Build ffplay with the native CMake backend: AUTO builds when SDL2 is available, ON requires SDL2, OFF disables it." FORCE)
set_property(CACHE FFMPEG_NATIVE_BUILD_FFPLAY PROPERTY STRINGS AUTO ON OFF)
...
if(FFMPEG_NATIVE_EFFECTIVE_ENABLE_ASM)
    if(NOT FFMPEG_X86ASM AND CMAKE_ASM_NASM_COMPILER AND EXISTS "${CMAKE_ASM_NASM_COMPILER}")
        set(FFMPEG_X86ASM "${CMAKE_ASM_NASM_COMPILER}" CACHE FILEPATH "Path to a NASM-compatible assembler executable such as nasm or yasm. Required for x86 assembly when it is not found in PATH." FORCE)
    endif()
