...
function(ffmpeg_add_external_project)
    ...
    if(NOT FFMPEG_MAKE_PROGRAM)
        ...
        set(FFMPEG_MAKE_PROGRAM "${_ffmpeg_make_program}" CACHE FILEPATH "Make program used to build FFmpeg" FORCE)
    endif()
    ProcessorCount(_ffmpeg_processor_count)
    if(_ffmpeg_processor_count EQUAL 0)
        set(_ffmpeg_processor_count "")
    endif()
    set(FFMPEG_MAKE_JOBS "${_ffmpeg_processor_count}" CACHE STRING "Optional parallel job count passed to make as -j<N>")

    if(NOT FFMPEG_CONFIGURE_SHELL)
        find_program(_ffmpeg_shell NAMES bash sh)
        if(_ffmpeg_shell)
            set(FFMPEG_CONFIGURE_SHELL "${_ffmpeg_shell}" CACHE FILEPATH "Shell used to run FFmpeg configure" FORCE)
        endif()
    endif()
