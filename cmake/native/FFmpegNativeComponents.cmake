...
function(_ffmpeg_native_append_windows_indev_components _enabled_var)
    if(NOT WIN32)
        set(${_enabled_var} "${${_enabled_var}}" PARENT_SCOPE)
        return()
    endif()

    if(FFMPEG_NATIVE_WINDOWS_DSHOW_INDEV_AVAILABLE)
        _ffmpeg_native_append_component_if_known(${_enabled_var} dshow_indev)
    endif()
    if(FFMPEG_NATIVE_WINDOWS_GDIGRAB_INDEV_AVAILABLE)
        _ffmpeg_native_append_component_if_known(${_enabled_var} gdigrab_indev)
    endif()
    if(FFMPEG_NATIVE_WINDOWS_VFWCAP_INDEV_AVAILABLE)
        _ffmpeg_native_append_component_if_known(${_enabled_var} vfwcap_indev)
    endif()
...
function(_ffmpeg_native_append_hardware_components _enabled_var _config_var)
    ...
    foreach(_ffmpeg_component IN LISTS FFMPEG_NATIVE_HWACCEL_LIST)
        ...
        elseif(_ffmpeg_component MATCHES "^(av1|h264|hevc|vp9)_vulkan_hwaccel$")
            _ffmpeg_native_append_hardware_if_backend(${_enabled_var} ${_config_var} "${_ffmpeg_component}" vulkan)
        endif()
    endforeach()
    ...
    foreach(_ffmpeg_component IN LISTS FFMPEG_NATIVE_ENCODER_LIST FFMPEG_NATIVE_DECODER_LIST)
        if(_ffmpeg_component MATCHES "_(amf|cuvid|d3d12va|mediacodec|mf|nvenc|qsv|rkmpp|vaapi|v4l2m2m|videotoolbox)_(encoder|decoder)$")
            _ffmpeg_native_append_hardware_if_backend(${_enabled_var} ${_config_var} "${_ffmpeg_component}" "${CMAKE_MATCH_1}")
        elseif(_ffmpeg_component MATCHES "^(av1|h264|hevc)_vulkan_encoder$")
            _ffmpeg_native_append_hardware_if_backend(${_enabled_var} ${_config_var} "${_ffmpeg_component}" vulkan)
        endif()
    endforeach()
